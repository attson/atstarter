...
function deleteButtons(mode: "trash" | "hard") {
  ...
  return [
    primary,
    { id: "cancel", label: T.cancel, kind: "secondary" as const },
  ];
}
...
<template>
  <div class="tree-wrap" @click="closeMenu">
    ...
    <div
      ...
      @click.stop
    >
      <button data-test="menu-new-file" @click="onMenuAction('newFile')">{{ T.newFile }}</button>
      <button data-test="menu-new-folder" @click="onMenuAction('newFolder')">{{ T.newFolder }}</button>
      <button data-test="menu-rename" @click="onMenuAction('rename')">{{ T.rename }}</button>
      <button data-test="menu-delete" @click="onMenuAction('delete')">{{ T.delete }}</button>
    </div>
    <ConfirmDialog
      v-if="deleteConfirm"
      :title="deleteConfirm.mode === 'hard'
        ? T.confirmHardDelete(baseName(deleteConfirm.node.path))
        : T.confirmTrash(baseName(deleteConfirm.node.path))"
