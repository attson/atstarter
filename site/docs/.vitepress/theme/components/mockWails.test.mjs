...
test('mock Wails app exposes realistic project and group data', async () => {
  ...
  assert.ok(projects.some((project) => project.name === 'ad-advertising-platform'))
...
test('mock Wails app updates command status and logs for the real app controls', async () => {
  const project = (await ListProjects()).find((item) => item.name === 'ad-advertising-platform')
  assert.ok(project)
...
test('mock Wails app returns branch names for project headers', async () => {
  assert.equal(await GetProjectBranch('/home/attson/GolandProjects/ad-advertising-platform'), 'master')
})
